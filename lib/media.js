...
function buildMask(src, dst, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    const p = spawn(cmd, args, { env: { ...process.env, BIRCH_FFMPEG: BIN.FFMPEG } });
    let err = '';
    p.stderr.on('data', d => {
...
async function probe(file) {
  const { stdout } = await run(BIN.FFPROBE, ['-v', 'quiet', '-print_format', 'json',
...
async function detectSilence(wav, minDur = 0.22) {
  ...
  let out = '';
  try {
    const r = await run(BIN.FFMPEG, ['-hide_banner', '-i', wav,
...
// the concat demuxer reads its list with its own escaping, where a Windows
// backslash starts an escape. ffmpeg takes forward slashes on Windows too.
const listed = p => process.platform === 'win32' ? p.replace(/\\/g, '/') : p;

const extractAudio = (src, dst) =>
  run(BIN.FFMPEG, ['-y', '-i', src, '-vn', '-ar', '16000', '-ac', '1', '-c:a', 'pcm_s16le', dst]);
...
function transcribe(wav, outBase, onProgress, onProc) {
  return new Promise((resolve, reject) => {
    const w = BIN.whisper();
    const p = spawn(w.cmd, [...w.args, '-m', MODEL, '-f', wav, '-oj', '-ojf', '-ml', '1',
...
async function faceBoxes(mask, times, W, H) {
  ...
  await run(BIN.FFMPEG, ['-y', '-loglevel', 'error', '-i', mask, '-vf', `select='${sel}',scale=120:-1`,
...
async function buildTrack(files, outdir, total) {
  ...
  for (const f of files) {
    ...
    if (a > at) L.push(`file '${listed(blank)}'`, `duration ${((a - at) / FPS).toFixed(4)}`);
    L.push(`file '${listed(f.file)}'`, `duration ${((b - a) / FPS).toFixed(4)}`);
    at = b;
  }
  if (fr(total) > at) L.push(`file '${listed(blank)}'`, `duration ${((fr(total) - at) / FPS).toFixed(4)}`);
  L.push(`file '${listed(blank)}'`);        // the concat demuxer drops the final entry's duration
  fs.writeFileSync(listPath, L.join('\n'));
  await run(BIN.FFMPEG, ['-y', '-f', 'concat', '-safe', '0', '-i', listPath,
...
module.exports = { frameFaces, frameGeometry, sfxInfo, speechLevel, whisper: BIN.whisper, MODEL, probe, extractAudio, transcribe, render, ranges, remap, buildMask,
