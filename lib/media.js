...
async function render(src, words, dst, opts = {}, onProgress, onProc) {
  ...
  if (opts.captions) {
    ...
    if (geo.bandH < H) {
      ...
      const cy = (geo.bandY + geo.bandH + (H - geo.bandY - geo.bandH) * 0.42) / H;
      caps.forEach(c => { c.pos = { cx: .5, cy, mult: 1 }; });
      ...
      caps.forEach(c => {
        ...
        const face = faceNear(faces, srcW ? srcW.start : 0);
        const n = (c.words || []).length || c.text.split(' ').length;
        const mult = n <= 2 ? 1.25 : 1.0;
        if (!face) { c.pos = { cx: .5, cy: .80, mult }; return; }
        const [, fy0, , fy1] = face;
        // the lower third where people read reels, dropped below the chin when the
        // face reaches down that far, and over the head only when there's no room
        let cy = Math.max(.70, fy1 + .08);
        if (cy > .84) cy = fy0 > .28 ? Math.max(.14, fy0 - .08) : .84;
        c.pos = { cx: .5, cy, mult };
      });
