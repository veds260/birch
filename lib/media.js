...
const { FFMPEG, FFPROBE, WHISPER } = require('./bin');
...
function buildMask(src, dst, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    const p = spawn(PERSONMASK, [src, dst, 'balanced']);
    let err = '';
    p.stderr.on('data', d => {
...
async function detectSilence(wav, minDur = 0.22) {
  try {
    const r = await run('python3', [path.join(__dirname, 'vad.py'), wav, String(minDur)]);
...
print(json.dumps(out))`;
  const { stdout } = await run('python3', ['-c', py, dir]).catch(() => ({ stdout: '[]' }));
...
const MAX_HOLD = { pop: 1.4, hook: 3.5, card: 3.0, tweet: 3.5, image: 3.0, broll: 6.0 };
...
async function render(src, words, dst, opts = {}, onProgress, onProc) {
  ...
  // story letterbox: the 9:16 frame keeps its size, the picture sits in a 2.35 band
  if (opts.letterbox) {
    g.push(`[vc]scale=${Math.round(opts.info?.width || 1080)}:-2,setsar=1[lbin]`);
    vlab = '[lbin]';
  }
  ...
  if (target || draft) {
    ...
    // a narrower frame than the footage keeps the speaker in it instead of the middle
    let cx = '(iw-ow)/2', cy = '(ih-oh)/2';
    const srcAr = (info.width || 1) / (info.height || 1);
    if (target && !opts.follow && opts.faces && opts.faces.length > 1 && Math.abs(srcAr - W / H) > 0.02) {
      const pts = [];
      for (const r of rs) for (let tt = r.start; tt < r.end; tt += 0.5) {
        const f = faceNear(opts.faces, tt);
        if (f) pts.push({ t: remap(tt, rs), cx: (f[0] + f[2]) / 2, cy: (f[1] + f[3]) / 2 });
      ...
      if (pts.length > 1) {
        const cwFrac = Math.min(1, (W / H) / srcAr), chFrac = Math.min(1, srcAr / (W / H));
        const px = panPath(pts, 'cx', cwFrac / 2), py = panPath(pts, 'cy', chFrac / 2);
        if (cwFrac < 1) cx = `iw*(${px})`;
        if (chFrac < 1) cy = `ih*(${py})`;
      }
    }
    g.push(`${vlab}crop='min(iw,ih*${ar})':'min(ih,iw/${ar})':'${cx}':'${cy}',scale=${W}:${H}:flags=${draft ? 'bilinear' : 'lanczos'},setsar=1[vr]`);
    vlab = '[vr]';
  }
  ...
  if (opts.letterbox) {
    const bh = Math.round(H * 0.5 / 2) * 2;      // picture band height, cinematic
    g.push(`${vlab}crop=iw:'min(ih,iw*${(bh / W).toFixed(5)})',scale=${W}:${bh},pad=${W}:${H}:0:${Math.round((H - bh) / 2)}:black[lbo]`);
    vlab = '[lbo]';
  }
  ...
  for (const s of sched) {
    ...
    if (ov.corner === 'top' || ov.slot === 'top') {          // inside the white panel
      ...
      const face = faceNear(opts.faces, s.srcT);
      let ih = sc * (ov.ar || 0.6);
  ...
  for (const [li, ls] of (opts.lists || []).entries()) {
    ...
    const outL = await new Promise((resolve, reject) => {
      const pr = spawn('python3', [path.join(__dirname, 'list.py')]);
      let o = '', e = '';
  ...
  if (opts.captions) {
    ...
    const quiet = sched.filter(s => s.ov.type === 'broll' || s.ov.type === 'card' ||
    ...
    if (opts.capDynamic && opts.faces && opts.faces.length) {
      // the vision pass gives the FACE, which is what to avoid. The person mask
      // includes the whole body, which pushed captions to silly places.
      caps.forEach(c => {
        const srcW = words.find(w => w.keep && remap(w.start, rs) >= c.start) || words[0];
        const face = faceNear(opts.faces, srcW ? srcW.start : 0);
        const n = (c.words || []).length || c.text.split(' ').length;
        const mult = n <= 2 ? 1.25 : 1.0;
        if (!face) { c.pos = { cx: .5, cy: .80, mult }; return; }
        const [, fy0, , fy1] = face;
        // under the chin with room, else above the head, else low
        const below = fy1 + .10, above = fy0 - .10;
        c.pos = { cx: .5, cy: below < .86 ? below : (above > .16 ? above : .82), mult };
      });
    ...
    const track = await buildCaptionTrack(caps, W, H, path.join(path.dirname(dst), 'caps'),
      { size: opts.capSize, bottom: opts.capBottom, boxOpacity: opts.capBox,
        style, highlight: opts.capHi, drift: opts.capDrift !== false }, total);
    if (track) {
      inputs.push('-i', track);
  ...
  const lead = (snd.lead ?? 0.1);                        // seconds the sound arrives before the picture
  const evAt = ov => { const w = words[ov.word] || words[0]; return remap(Math.max(w.start, rs[0].start), rs); };
  if (snd.sfx !== false) {
    const map = snd.map || { pop: 'thud', hook: 'click', card: 'whoosh', broll: 'whoosh', tweet: 'whoosh', image: 'pop', list: 'pop', reframe: 'swoosh' };
    for (const ov of (opts.overlays || [])) {
      const key = ov.type === 'pop' ? (ov.kind === 'pop' ? 'pop' : ov.kind === 'comment' ? 'tweet' : ov.kind === 'namecard' ? 'card' : 'hook') : ov.type;
      const n = map[key]; const f = n && sfxFile(n);
      if (f) hits.push({ file: f, at: Math.max(0, evAt(ov) - lead), gain: snd.sfxGain ?? 0.9 });
    }
    for (const ls of (opts.lists || [])) for (const ln of ls.lines) {
      const w = words[ln.word]; if (!w) continue;
      const f = sfxFile(map.list || 'pop'); if (f) hits.push({ file: f, at: Math.max(0, remap(Math.max(w.start, rs[0].start), rs) - lead), gain: (snd.sfxGain ?? 0.9) * 0.7 });
    }
    for (const cw of (opts.cows || [])) { const w = words[cw.word]; if (!w) continue;
      const f = sfxFile('hit'); const f2 = sfxFile('riser');
      const at0 = Math.max(0, remap(Math.max(w.start, rs[0].start), rs) - lead);
      if (f) hits.push({ file: f, at: at0, gain: (snd.sfxGain ?? 0.9) * 1.1 });
      if (f2) hits.push({ file: f2, at: Math.max(0, at0 - 0.9), gain: (snd.sfxGain ?? 0.9) * 0.6 }); }
    for (const rf of (opts.reframes || [])) { const w = words[rf.word]; if (!w) continue;
      const f = sfxFile(map.reframe || 'swoosh'); if (f) hits.push({ file: f, at: Math.max(0, remap(Math.max(w.start, rs[0].start), rs) - lead), gain: (snd.sfxGain ?? 0.9) * 0.8 }); }
  }
  const mixIns = [];
  ...
  hits.forEach((h, i) => {
    ...
    g.push(`[${hi}:a]volume=${h.gain.toFixed(2)},adelay=${ms}|${ms},apad=whole_dur=${total0.toFixed(3)}[hit${i}]`);
  ...
  // quality difference does not show on a phone screen
  const crf = opts.crf ?? 18;
  const vt = opts.encoder !== 'x264';
  const venc = vt
...
module.exports = { WHISPER, MODEL, probe, extractAudio, transcribe, render, ranges, remap, buildMask,
