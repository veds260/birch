...
async function seat(words, silences, duration, wav, o = {}) {
  ...
  if (!words.length || !isl.length || !whisper || !model) return align(words, silences, duration);
  ...
  try {
    ...
    for (let k = 0; k < files.length; k += 150) {
      const args = ['-m', model, '-oj', '-ojf', '-ml', '1', '-sow', '-wt', '0.01', '-t', String(o.threads || 6)];
      for (const f of files.slice(k, k + 150)) args.push('-f', f);
      await new Promise((res, rej) => execFile(whisper, args, { maxBuffer: 1 << 26 }, e => e ? rej(e) : res()));
    }
