...
def main():
    ...
    if head:
        f = fonts.load(FACES.get(c.get("face", "heavy")), int(c.get("headSize") or H * 0.058))
    ...
    if lines:
        f = fonts.load(FACES.get(c.get("listFace", "bold")), int(c.get("listSize") or H * 0.036))
    ...
    if body:
        f = fonts.load(FACES.get("medium"), int(c.get("bodySize") or H * 0.03))
    ...
    if hi:   # one word or phrase in the accent colour, big, the thing the card is about
        f = fonts.load(FACES["heavy"], int(H * 0.05))
