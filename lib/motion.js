...
function py(script, cfg) {
  return new Promise((resolve, reject) => {
    const p = spawn(require('./bin').python(), [path.join(__dirname, script)]);
    let out = '', err = '';
...
async function gather(ins, dir) {
  ...
  if (ins.type === 'site') {
    if (!CHROME) throw new Error('no Chrome, Chromium or Edge on this machine to take the screenshot with');
...
async function render(ins, { W = 1080, H = 1920, y = 0.3, workdir }) {
  ...
  const r = await run(require('./bin').find('npx'), ['--yes', 'hyperframes', 'render', dir, '--format', 'mov', '--quality', 'draft',
