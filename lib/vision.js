...
// The Swift tool is macOS only. Everywhere else, and when BIRCH_NO_SWIFT is set,
// the same job is done by lib/vision_fallback.py on mediapipe.
const swiftOK = () => {
  if (process.env.BIRCH_NO_SWIFT) return false;
  try { fs.accessSync(TOOL, fs.constants.X_OK); return true; } catch { return false; }
};
const engine = () => swiftOK() ? 'apple vision' : 'mediapipe';
const cmdFor = rest => swiftOK()
  ? [TOOL, rest]
  : [process.env.BIRCH_PYTHON || require('./bin').python(), [path.join(__dirname, 'vision_fallback.py'), ...rest]];

function analyse(src, out, onProgress) {
  return new Promise((resolve, reject) => {
    const [cmd, args] = cmdFor([src, out, '1']);
    console.log('vision: reading the footage with ' + engine());
...
function cuts(src) {
  return new Promise(resolve => {
    execFile(BIN.FFMPEG, ['-hide_banner', '-i', src, '-vf', "select='gt(scene,0.35)',showinfo", '-f', 'null', '-'], { maxBuffer: 1 << 26 },
...
module.exports = { analyse, load, cuts, at, faceAt, screenSpans, framing, summary, speakerTrack, engine };
