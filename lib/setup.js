...
const LINUX_PKGS = [
  { has: 'apt-get', cmd: 'sudo apt-get install -y ffmpeg cmake build-essential python3-pip' },
  { has: 'dnf', cmd: 'sudo dnf install -y ffmpeg cmake gcc-c++ make python3-pip' },
  { has: 'pacman', cmd: 'sudo pacman -S --needed ffmpeg cmake base-devel python-pip' },
  { has: 'zypper', cmd: 'sudo zypper install -y ffmpeg cmake gcc-c++ make python3-pip' },
];
...
async function status() {
  ...
  // on Linux the face work is done by python, on a Mac by the built Swift tools
  ...
  const needs = MAC ? null : await linuxCommand();
  const steps = {
    github: { done: !!cfg.github, login: cfg.github || null, suggest: ghLogin, canClick: !!ghLogin },
    tools: { done: !!ffmpeg && !!whisper, ffmpeg: !!ffmpeg, whisper: !!whisper, brew: !!brew, mac: MAC, command: needs },
    model: { done: modelSize > MODEL_BYTES * 0.98, have: modelSize, total: MODEL_BYTES },
    vision: { done: !!looker.ok && pil.ok, pillow: pil.ok, mac: MAC },
...
function run(step, progress) {
  ...
  if (step === 'vision') return (async () => {
    ...
    // Linux has no Vision framework, so the faces, lips and cut-out come from python
    ...
    if (!ok.ok) throw new Error('The face packages did not import: ' + ok.err.slice(-160));
...
async function pythonPackages(pkgs, progress, base = 5) {
  const missing = [];
  for (const p of pkgs) {
    const mod = { Pillow: 'PIL', 'opencv-python-headless': 'cv2', numpy: 'numpy', mediapipe: 'mediapipe' }[p] || p;
    const r = await sh(BIN.python(), ['-c', 'import ' + mod], { timeout: 60000 });
    if (!r.ok) missing.push(p);
  }
  if (!missing.length) return;
  const venv = path.join(ROOT, 'venv');
  let py = BIN.python();
  const tryInstall = async () => sh(py, ['-m', 'pip', 'install', '--quiet', ...missing], { timeout: 900000 });
  progress(base, 'installing ' + missing.join(', '));
  let r = await tryInstall();
  if (!r.ok && !exists(path.join(venv, 'bin', 'python3'))) {
