...
async function status() {
  ...
  const [ffmpeg, whisper, brew, claude, gh] = await Promise.all(['ffmpeg', 'whisper-cli', 'brew', 'claude', 'gh'].map(which));
  const pil = await sh('python3', ['-c', 'import PIL']);
  // `claude mcp list` health-checks every server and takes seconds, so read the config
  let mcp = false;
  try { mcp = !!JSON.parse(fs.readFileSync(path.join(require('os').homedir(), '.claude.json'), 'utf8')).mcpServers?.birch; } catch {}
  let ghLogin = null;
  ...
  const steps = {
    ...
    claude: { done: !!claude, optional: true },
    mcp: { done: mcp, optional: true, possible: !!claude },
  };
...
function run(step, progress) {
  if (step === 'tools') return (async () => {
    ...
    if (!brew) throw new Error('Homebrew is needed first. Install it from brew.sh, then press this again.');
    const want = [];
  ...
  if (step === 'mcp') return (async () => {
    ...
    const r = await sh(claude, ['mcp', 'add', '--scope', 'user', 'birch', '--', path.join(ROOT, 'bin', 'birch'), 'mcp'], { timeout: 30000 });
    if (!r.ok && !/already exists/i.test(r.err + r.out)) throw new Error(r.err.slice(-200) || 'claude mcp add failed');
  })();
