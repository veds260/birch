...
// the same job is done by lib/lips_fallback.py on mediapipe.
const swiftOK = () => {
...
function scan(src, out, onProgress) {
  return new Promise((resolve, reject) => {
    const [cmd, args] = swiftOK()
      ? [TOOL, [src, out, '5']]
      : [process.env.BIRCH_PYTHON || require('./bin').python(), [path.join(__dirname, 'lips_fallback.py'), src, out, '5']];
    console.log('talkers: reading mouths with ' + engine());
...
module.exports = { scan, load, tracks, speakerPath, engine };
