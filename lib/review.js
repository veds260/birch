...
const os = require('os');
const { FFMPEG, FFPROBE } = require('./bin');
...
async function frameStats(file, times, dir) {
  ...
  for (const [i, t] of times.entries())
    await run(FFMPEG, ['-y', '-loglevel', 'error', '-ss', String(t), '-i', file, '-frames:v', '1',
...
print(json.dumps(out))`;
  const { so } = await run('python3', ['-c', py, dir]).catch(() => ({ so: '[]' }));
  return JSON.parse(so || '[]');
}

async function audio(file) {
  const { se } = await run(FFMPEG, ['-hide_banner', '-i', file, '-af', 'ebur128=peak=true', '-f', 'null', '-']).catch(e => ({ se: String(e.message) }));
...
async function review(file, plan) {
  ...
  const { so } = await run(FFPROBE, ['-v', 'quiet', '-print_format', 'json', '-show_format', '-show_streams', file]);
