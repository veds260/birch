  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        name: { type: 'string', description: 'adds a name card with this name. Leave it out for a normal reel, which has none' },
  ...
  const handlers = {
    async birch_setup({ open = true }) {
      ...
      const lines = [
        ...
        `AI for planning: ${s.ai.done ? (s.ai.active === 'claude' ? 'Claude' : 'ChatGPT') + ' connected' : 'not connected (optional, sign in on the setup page)'}`,
      ];
    ...
    async birch_cut(a) {
      ...
      // no name card unless asked for, or the clip introduces a guest
      const wantCard = !!a.name || !!(direction && direction.nameCard);
      if (name && wantCard) await request('POST', '/api/birch/namecard', { id, word: firstKept, seconds: 3, name, role });
      ...
      return [
        `Done: ${out}`,
        `speaker: ${name || 'not named in the clip'}${direction && direction.speaker.how && !a.name ? ' (' + direction.speaker.how + ')' : ''}${name && wantCard ? ', name card added' : ''}`,
